...
void JobNode::Create(JobBucket* bucket, StackObject<Runnable<void()>> const& runnable, JobSystem* sys, JobHandle const* dependedJobs, uint dependCount)
{
	std::mutex* threadMtx = &sys->threadMtx;
	targetDepending = dependCount;
	this->threadMtx = threadMtx;
	memcpy(&this->runnable, runnable, sizeof(Runnable<void()>));
...
void JobNode::CreateParallel(JobBucket* bucket, StackObject<Runnable<void(uint)>> const& runnable, uint parallelStart, uint parallelEnd, JobSystem* sys, JobHandle const* dependedJobs, uint dependCount)
{
	std::mutex* threadMtx = &sys->threadMtx;
	targetDepending = dependCount;
	this->threadMtx = threadMtx;
