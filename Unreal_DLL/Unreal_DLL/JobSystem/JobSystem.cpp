...
START:
	...
	executingNode.ResizeAndClear(bucket->allJobNodes.size());
	...
	currentBucketPos++;
	uint32_t size = executingNode.GetSize();
...
JobBucket* JobSystem::GetJobBucket()
{
	...
	{
		JobBucket* bucket = vengine_new<JobBucket>(this);
...
void JobSystem::ExecuteBucket(JobBucket** bucket, uint32_t bucketCount)
{
	...
	jobSystemStart = true;
	UpdateNewBucket();
	allocatorSwitcher = !allocatorSwitcher;
	//FreeAllMemory();
