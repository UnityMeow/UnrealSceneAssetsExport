...
namespace neb
{
	...
	void CJsonObject::Clear()
	{
		...
		m_mapJsonArrayRef.IterateAll([&](uint64_t i, uint32_t const& key, CJsonObject*& value) -> void
		{
			if (value != nullptr)
			{
				delete (value);
		...
		m_mapJsonObjectRef.IterateAll([&](uint64_t i, vengine::string const& key, CJsonObject*& value) -> void
		{
			if (value != nullptr)
			{
				delete value;
	...
	bool CJsonObject::Get(const vengine::string& strKey, uint32& uiValue) const
	{
		cJSON* pJsonStruct = NULL;
		if (m_pJsonData != NULL)
		{
	...
	bool CJsonObject::Add(const vengine::string& strKey, uint32 uiValue)
	{
		cJSON* pFocusData = NULL;
		if (m_pJsonData != NULL)
		{
	...
	bool CJsonObject::Replace(const vengine::string& strKey, uint32 uiValue)
	{
		cJSON* pFocusData = NULL;
		if (m_pJsonData == NULL)
		{
	...
	bool CJsonObject::Get(int iWhich, vengine::string& strValue) const
	{
		cJSON* pJsonStruct = NULL;
		if (m_pJsonData != NULL)
		{
	...
	bool CJsonObject::Add(const CJsonObject& oJsonObject)
	{
		...
		m_mapJsonArrayRef.IterateAll([&](uint32_t i, uint32_t key, CJsonObject*& value)->void
		{
			if (key >= uiLastIndex)
			{
				if (value != NULL)
	...
	bool CJsonObject::Replace(int iWhich, const vengine::string& strValue)
	{
		cJSON* pFocusData = NULL;
		if (m_pJsonData == NULL)
		{
...
bool ReadJson(const vengine::string& filePath, neb::CJsonObject* cjsonObj)
{
	std::ifstream ifs(filePath.data());
	if (!ifs) return false;
	ifs.seekg(0, std::ios::end);
