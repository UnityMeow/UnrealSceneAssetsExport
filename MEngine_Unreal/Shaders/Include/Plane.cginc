    ...
    float4 TetLerp(Tet tm, float4 value0, float4 value1, float4 value2, float4 value3, float3 position, out bool isInside)
    {
        float4 pWeight = float4(GetDistanceToPlane(tm.plane0, position),
        GetDistanceToPlane(tm.plane1, position),
        GetDistanceToPlane(tm.plane2, position),
        GetDistanceToPlane(tm.plane3, position));
        isInside = all(pWeight > 0);
        if(isInside)
        {
            pWeight *= tm.planeSizes;
            return (value0 * pWeight.x + value1 * pWeight.y + value2 * pWeight.z + value3 * pWeight.w) / dot(pWeight, 1);
        }
        else
        {
            return 0;
